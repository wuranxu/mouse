...
func (s *statsEntry) logResponseTime(responseTime int64) {
	...
	if responseTime < 100 {
		...
		roundedResponseTime = int64(utils.Round(float64(responseTime), .5, -1))
	} else if responseTime < 10000 {
		roundedResponseTime = int64(utils.Round(float64(responseTime), .5, -2))
	} else {
		roundedResponseTime = int64(utils.Round(float64(responseTime), .5, -3))
	}
